    ...
    while num > MAX_COMP:
        ...
        for i in range(len(labels)):
            ...
            if np.sum(in_cluster) > 1:
                ...
                component_id = np.flatnonzero(in_cluster)[0]
                new_w_merged[i] = w_merged[component_id]
                new_means_merged[i] = means_merged[component_id]
                new_covs_merged[i] = covs_merged[component_id]
